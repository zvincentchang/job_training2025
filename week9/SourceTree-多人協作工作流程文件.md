...
> 生產環境（main）發現嚴重 Bug，需立即修復，不經過 develop 流程。

### 9.1 從 main 建立 Hotfix 分支

```
SourceTree → 切換到 main 分支
  左側 BRANCHES → 雙擊 main

建立 hotfix 分支：
  工具列 → Branch → New Branch: hotfix/fix-null-pointer
  → Checkout New Branch: ✅ → Create Branch
```
...
public class ClassA {
    public String getInfo() {
        return name + " - v" + version;  // name 可能為 null
    }
}
```
...
```
Commit message: "fix: Handle null name in ClassA.getInfo()"
→ Push → hotfix/fix-null-pointer
→ 建立 PR → target: main
...
合併 hotfix：
  右鍵 hotfix/fix-null-pointer → Merge into current branch
  → Commit → Push develop
```
...
```
左側 BRANCHES → 右鍵 hotfix/fix-null-pointer → Delete
```
...
```
切換到 develop（確認所有功能已合併完整）

工具列 → Branch → New Branch: release/1.0.0
→ Checkout New Branch: ✅ → Create Branch → Push
```

### 10.2 Release 分支允許的操作
...
```
Commit message: "chore(release): Bump version to 1.0.0"
```

### 10.4 Merge release 到 main 和 develop

**合併到 main：**
```
切換到 main → 右鍵 release/1.0.0 → Merge into current branch
→ Push main
```

**合併回 develop：**
```
切換到 develop → 右鍵 release/1.0.0 → Merge into current branch
→ Push develop
```
...
> 每次正式 Release 後，在 main 的對應 commit 打上 Tag，方便日後回溯。

### 11.1 在 SourceTree 建立 Tag

```
切換到 main（已合併 release/1.0.0）

左側 Log/History → 右鍵最新 commit → Tag...
  Tag Name: v1.0.0
  Description: "First stable release - includes ClassA, ClassB, ClassC"
  → Add Tag → Push Tags to Remote
```
...
```
開始工作前，先同步遠端最新代碼：
  工具列 → Pull → Remote: origin → Branch: develop → Pull
```
...
```
需要緊急切換分支但目前修改未完成時：
  工具列 → Stash → Stash message: "WIP: ClassA feature in progress"
  → Stash

切換完成後恢復：
  左側 STASHES → 雙擊 Stash 項目 → Pop Stash
```

### 12.3 Revert 撤銷 Commit

```
後悔某次 Commit（不修改歷史，安全做法）：
  左側 Log/History → 右鍵目標 Commit → Reverse Commit...
  → 會產生一個新的 Commit 來抵消變更
```
...
| 規則 | 說明 |
|---|---|
| Protect main branch | 禁止直接 push，只允許 PR merge |
| Require PR review | 至少 1～2 位 Reviewer Approve |
| CI/CD 必須通過 | 自動化測試 pass 才可 merge |
| Delete branch after merge | PR 合併後自動刪除功能分支 |
| Branch naming convention | 強制 feature/、hotfix/、release/ 命名 |

### 12.5 完整工作流程速查

```
每日開始工作：
  1. git pull develop（SourceTree → Pull）
  2. 在 feature/xxx 分支開發
  3. 小步提交（每個邏輯變更一個 commit）
  4. 推送 feature 分支（Push）

功能完成：
  5. 建立 PR → target: develop
  6. 等待 Code Review
  7. 解決 Review 意見 → 補充 commit
  8. Approve → Merge → Delete branch

上線準備：
  9. 建立 release/x.x.x → 測試 → 修正
...
develop:         B ──────────────────────────────────►
                  \          \         \
feature/A-v1:      C─C─C ──► PR#1 Merged
feature/B-v1:        D─D─D ──► PR#2 Merged
feature/C-v1:          E─E─E ──► PR#3 Merged
```
...
| 分支 | 檔案 | 功能說明 |
|---|---|---|
| `main` | HelloWorld.java | 程式進入點 |
| `feature/A-v1` | ClassA.java | 名稱與版本管理 |
| `feature/B-v1` | ClassB.java | 類別處理邏輯 |
| `feature/C-v1` | ClassC.java | 狀態切換功能 |

---

*文件版本：v1.0 ｜ 最後更新：2026-05-02*
